...
class Contact:
    ...
    def pack_in_tk_frame(self, master_frame, row_number):
        ...
        first_name = Label(row_frame, text=self.first_name, bg=background, width=15)
        first_name.grid(row=self.row_number, column=0)
        family_name = Label(row_frame, text=self.family_name, bg=background, width=15)
        family_name.grid(row=self.row_number, column=1)
        address = Label(row_frame, text=self.address, bg=background, width=45)
        address.grid(row=row_number, column=2)
        phone_number = Label(row_frame, text=self.phone_number, bg=background, width=15)
        phone_number.grid(row=self.row_number, column=3)
        email_address = Label(row_frame, text=self.email_address, bg=background, width=45)
        email_address.grid(row=self.row_number, column=4)
        
        modification_icone = PhotoImage(file="modification.png")
        modification_button = Button(row_frame, image=modification_icone, bg="#EEEEEE", width=30, command=self.contact_modification)
        modification_button.grid(row=0, column=5, padx=5)

        delete_icone = PhotoImage(file="trash.png")
        delete_button = Button(row_frame, image=delete_icone, bg="#EEEEEE", width=30, command=self.delete_contact)
